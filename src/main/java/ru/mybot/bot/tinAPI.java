...
// итерация по массиву объектов "positions"
    ...
    public static String getActions() throws IOException, ParseException {
        ...
        var valNow = api.getOperationsService().getPortfolio("number_id_invest").join().getTotalAmountPortfolio().getValue();
        var orderValMin = api.getOperationsService().getPortfolio("number_id_invest").join().getPositions();
        var orderValMon = api.getOperationsService().getWithdrawLimits("number_id_invest").join();
        ...
        for (var orderVal : api.getOperationsService().getPortfolio("number_id_invest").join().getPositions()) {
